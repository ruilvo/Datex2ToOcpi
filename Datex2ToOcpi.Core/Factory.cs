...
{
    /// <summary>
    /// Deserialize a Datex II EnergyInfrastructureTable from a stream.
    ///
    /// The stream should contain a valid Datex II XML document with
    /// an ns6:EnergyInfrastructureTablePublication as the payload.
    /// </summary>
    ...
    public static EnergyInfrastructureTable EnergyInfraTableFromStream(Stream stream)
    {
        try
        {
            ...
            var payload = serializer.Deserialize(stream) as Models.Datex2.Common.PayloadPublication
                          ?? throw new InvalidOperationException("Deserialization returned a null PayloadPublication.");
